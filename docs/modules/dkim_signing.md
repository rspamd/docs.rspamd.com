...
Rspamd supports both HashiCorp Vault KV secrets engine version 1 (default) and version 2. Use the `vault_kv_version` option to select the version matching your Vault mount. For more information on Vault secrets engines, see the [guide](https://learn.hashicorp.com/vault/getting-started/secrets-engines).
...
# vault_path = "dkim"; # KV mount path (default: "dkim")
# vault_kv_version = 1; # KV secrets engine version: 1 (default) or 2
# no_ssl_verify = false; # Set true to disable TLS certificate verification
# http_timeout = 5.0; # Vault HTTP request timeout in seconds (default: 5.0)
~~~
...
The roll subcommand can also remove all expired keys from the vault.


## Advanced options

The following options are available in `lua_dkim_tools` and apply to both `dkim_signing` and `arc` modules.

### Domain selection

| Option | Default | Description |
| :----- | :------ | :---------- |
| `use_domain_sign_inbound` | — | Domain source to use when the message is inbound (not from a local IP and not authenticated). Accepts the same values as `use_domain`: `"header"`, `"envelope"`, `"auth"`, `"recipient"`, or a literal domain name. |
| `use_domain_custom` | — | A Lua string that evaluates to a function `function(task) return domain end`. Called to determine the signing domain when none of the other `use_domain_sign_*` conditions match. Allows fully custom domain selection logic. |

### Redis selector prefix

| Option | Default | Description |
| :----- | :------ | :---------- |
| `selector_prefix` | — | When set and `use_redis = true`, Rspamd performs an `HGET <selector_prefix> <domain>` lookup to resolve the per-domain selector before fetching the key. If not set, the globally configured `selector` is used directly. |

### HTTP headers fallback

| Option | Default | Description |
| :----- | :------ | :---------- |
| `allow_headers_fallback` | `false` | When `use_http_headers = true` and the required HTTP request headers are absent, normally signing is skipped. Setting this to `true` causes Rspamd to fall back to the standard signing logic instead. |

### DKIM violation check

| Option | Default | Description |
| :----- | :------ | :---------- |
| `check_violation` | `false` | When `true`, skip signing if `R_DKIM_REJECT` (or `ARC_REJECT` for ARC) is present on the task. This prevents re-signing messages that have already failed DKIM verification. |

### Vault options

| Option | Default | Description |
| :----- | :------ | :---------- |
| `vault_path` | `"dkim"` | The KV mount path within Vault under which DKIM keys are stored. |
| `vault_kv_version` | `1` | Vault KV secrets engine version. Set to `2` to use the KV v2 API (Rspamd automatically rewrites the request path to include the required `/data/` segment). |
| `no_ssl_verify` | `false` | Disable TLS certificate verification for Vault HTTPS connections. Use only in controlled environments. |
| `http_timeout` | `5.0` | Timeout in seconds for HTTP requests to Vault. |
